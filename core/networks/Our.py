...
class TransformerBlock(nn.Module):
    ...
    def __init__(self, tokensize, hidden=128, num_head=4, dropout=0., zone=2):
        super().__init__()
...
class DecoderEffcientAttention(nn.Module):
    def __init__(self, hidden_dim=96, cross_scale=True):
        super(DecoderEffcientAttention, self).__init__()
...
class DecoderTransformerBlock(nn.Module):
    def __init__(self, hidden=96, dropout=0.0, cross_scale=True):
        ...
        self.attention = DecoderEffcientAttention(hidden_dim=hidden, cross_scale=cross_scale)
...
class DecoderTransformer(nn.Module):
    def __init__(self, num_layers=4, hidden=96, cross_scale=True):
        ...
        layer = DecoderTransformerBlock(hidden=hidden, dropout=0.0, cross_scale=cross_scale)
...
class CustomDecoder(nn.Module):
    def __init__(self, num_layers=4):
        ...
        self.stage1 = DecoderTransformer(num_layers=4, hidden=96, cross_scale=False)
        self.stage2 = DecoderTransformer(num_layers=4, hidden=96, cross_scale=True)
        self.stage3 = DecoderTransformer(num_layers=4, hidden=96, cross_scale=True)
        self.stage4 = DecoderTransformer(num_layers=4, hidden=96, cross_scale=True)
...
class DomainFusionModule(nn.Module):
    def __init__(self, in_dim, hidden_dim=96):
        super(DomainFusionModule, self).__init__()
        ...
        self.attn_fusion = STAFM(hidden_dim, ratio=4)
        self.sigmoid = nn.Sigmoid()
...
# 测试T的影响(9: 89.5) (1: 89.35) (3: 89.6) (5: 89.6) (7: 89.7) (8: 89.65) (9: 89.7) (10: 89.7)
# (11: 89.7) (12: 89.7) (18: 89.6)
class CLIPVOS(nn.Module):
    def __init__(self,
                 config=None,
                 in_channels: List[int] = [96, 192, 384, 768],
                 out_channels: List[int] = [96, 96, 96, 96]):
        super(CLIPVOS, self).__init__()
        self.out_channels = out_channels
        self.config = config
        self.frame_nums = self.config['train_frames']
        self.inference_frame_nums = self.config['clip_length']
        self.backbone = build_encoder(name=config['encoder_name'])
        hidden_dim = 96
        self.fusion1 = DomainFusionModule(in_dim=in_channels[0], hidden_dim=hidden_dim)
        self.fusion2 = DomainFusionModule(in_dim=in_channels[1], hidden_dim=hidden_dim)
        self.fusion3 = DomainFusionModule(in_dim=in_channels[2], hidden_dim=hidden_dim)
        self.fusion4 = DomainFusionModule(in_dim=in_channels[3], hidden_dim=hidden_dim)
        stack_num = 8
        num_head = 4
        blocks3 = []
        blocks4 = []
        ...
        for _ in range(stack_num // 2):
            blocks3.append(
                TransformerBlock(tokensize=(32, 32), hidden=hidden_dim, num_head=num_head, dropout=dropout, zone=2))
            blocks4.append(
                TransformerBlock(tokensize=(16, 16), hidden=hidden_dim, num_head=num_head, dropout=dropout, zone=2))
        self.transformer3 = nn.Sequential(*blocks3)
        self.transformer4 = nn.Sequential(*blocks4)
        self.decoder = CustomDecoder()
        self.fpn = FPN(out_channels)
        self.cls2 = nn.Sequential(nn.Conv2d(out_channels[0], out_channels[0], kernel_size=3, padding=1),
                                  nn.BatchNorm2d(out_channels[0]),
                                  nn.ReLU(),
                                  nn.Conv2d(out_channels[0], 1, kernel_size=1))
        self.cls3 = nn.Sequential(nn.Conv2d(out_channels[0], out_channels[0], kernel_size=3, padding=1),
                                  nn.BatchNorm2d(out_channels[0]),
                                  nn.ReLU(),
                                  nn.Conv2d(out_channels[0], 1, kernel_size=1))
        self.cls4 = nn.Sequential(nn.Conv2d(out_channels[0], out_channels[0], kernel_size=3, padding=1),
                                  nn.BatchNorm2d(out_channels[0]),
                                  nn.ReLU(),
                                  nn.Conv2d(out_channels[0], 1, kernel_size=1))
        self.cls5 = nn.Sequential(nn.Conv2d(out_channels[0], out_channels[0], kernel_size=3, padding=1),
                                  nn.BatchNorm2d(out_channels[0]),
                                  nn.ReLU(),
                                  nn.Conv2d(out_channels[0], 1, kernel_size=1))
        self.sigmoid = nn.Sigmoid()
    ...
    def forward(self, imgs, flows):
        ...
        # Feature extract
        img_conv1, img_conv2, img_conv3, img_conv4 = self.backbone(imgs)  # ConvNext 4x, 8x, 16x, 32x
        flow_conv1, flow_conv2, flow_conv3, flow_conv4 = self.backbone(flows)
        # Feature simple fusion
