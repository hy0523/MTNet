...
import torch
from core.dataloader.range_transform import inv_im_trans
...
def plot_feat_map(feats, id=0, size=(256, 256), save_path=None):
    feats = F.interpolate(feats, size=size, mode='bilinear', align_corners=True)  # [B,C,H,W]
    fea_map = feats[id]
    fm = torch.sum(fea_map, 0)
    f = make_grid(fm, nrow=8, padding=2, normalize=True, range=None, scale_each=False, pad_value=0)
    f_map = np.array(f.cpu())
    if save_path:
        plt.imshow(f_map[0])
        plt.axis('off')
...
def show_tensor(tensor, id):
    img = tensor[id].mean(0).cpu().detach().numpy()
    plt.imshow(img)
    plt.show()
...
def pool_pairs(images, size, so):
    ...
    # limit number of images to saves disk space
    b = max(2, b)
