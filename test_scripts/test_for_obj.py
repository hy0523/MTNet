...
def get_method_score_dict(
    ...
    video_score_list = Parallel(n_jobs=10)(  # 10
...
def convert_data_dict_to_table(data_dict: dict, video_name_list: list):
    ...
    meta_class_dict = {'aeroplane': [],
                       ...
                       'train': []}
    for video_name in video_name_list:
        meta_video_name = re.sub(r'[0-9]+', '', video_name)
        if meta_video_name in meta_class_dict.keys():
            meta_class_dict[meta_video_name].append(float(data_dict[video_name]['mean'][0]))
    for key, value in meta_class_dict.items():
        meta_class_dict[key] = float('{:.3f}'.format(sum(value) / len(value)))
    meta_class_score = []
    [meta_class_score.append(meta_class_dict[key]) for key in meta_class_dict.keys()]
    meta_class_dict['Average'] = float('{:.3f}'.format(sum(meta_class_score) / 10))
    return meta_class_dict
...
if __name__ == "__main__":
    ...
    parser.add_argument('--gt_path', default="../data/YO2SEG/val/mask")
    args = parser.parse_args()
    config = vars(args)
    mask_data_root = config['gt_path']
    eval_method_from_data(
        method_pre_path=config['result_path'],
        ignore_tail=False,  # False
        ignore_head=False,  # False
